...
from .utils import add_random_obstacles
...
def random_grid_terrain(
    ...
    # generate the border
    border_width = (cfg.size[0] - num_boxes_x * cfg.grid_width, cfg.size[1] - num_boxes_y * cfg.grid_width)
    if border_width[0] > 0 and border_width[1] > 0:
        # compute parameters for the border
        border_center = (0.5 * cfg.size[0], 0.5 * cfg.size[1], -terrain_height / 2)
        border_inner_size = (cfg.size[0] - border_width[0], cfg.size[1] - border_width[1])
    ...
    offsets[:, 0] += border_width[0] / 2
    offsets[:, 1] += border_width[1] / 2
    ...
    if cfg.holes:
        ...
        mask_x = torch.logical_and(
            (vertices[:, :, 0] > (cfg.size[0] - border_width[0] - cfg.platform_width) / 2).all(dim=1),
            (vertices[:, :, 0] < (cfg.size[0] + border_width[0] + cfg.platform_width) / 2).all(dim=1),
        )
        vertices_x = vertices[mask_x]
        # -- y-axis
        mask_y = torch.logical_and(
            (vertices[:, :, 1] > (cfg.size[1] - border_width[1] - cfg.platform_width) / 2).all(dim=1),
            (vertices[:, :, 1] < (cfg.size[1] + border_width[1] + cfg.platform_width) / 2).all(dim=1),
        )
