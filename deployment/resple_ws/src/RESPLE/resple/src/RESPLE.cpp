    ...
    RESPLE(rclcpp::Node::SharedPtr& nh) 
    {
        ...
        for (const auto& [lidar_name, lidar] : lidars) {
            if (!lidar.type.compare("HAP360")) {
                ...
                std::cerr << "Does not support LiDAR type: " << lidar.type << std::endl;
                assert(false);
            }
        }        
    }
        ...
        while (true) {      
            ...
            while (collectMeasurements()) {
                ...
                if (spline->numKnots() > max_spl_knots) {
                    int64_t num_knot = spline->numKnots();
                    int64_t dt_spl_ns = spline->getKnotTimeIntervalNs();
                    spl_window_st_ns = spline->getKnotTimeNs(num_knot - 5); 
                    pubOdom();
                    while (spline->numKnots() > int64_t(5e8/dt_spl_ns)) {
                        spline->removeOneOldState();
                    }                    
...
private:
    ...
    int64_t spl_window_st_ns = 0;
    int64_t last_odom_ns = 0;
    ...
    void pubOdom()
    {
        int64_t t_ns = std::min(std::max(spl_window_st_ns, spline->minTimeNs()), spline->maxTimeNs());
        if (t_ns < last_odom_ns + 5e7) {
            return;
        }
        ...
        geometry_msgs::msg::PoseStamped odom_pose = CommonUtils::pose2msg(t_ns, t_interp, orient_interp);
