...
class SplineState
{
  ...
  public:
    ...
    void removeOneOldState()
    {
        ...
        ort_delta_idle = {ort_delta_idle[1], ort_delta_idle[2], ort_delta.front()};
        q_knots.pop_front();
        t_knots.pop_front();
        ort_delta.pop_front();
        num_knot--;
    ...
    void updateKnots(SplineState* other)
    {
        ...
        for (int64_t i = 0; i < num_knots_other; i++) {
            if (i + other->start_i - this->start_i < num_knot) {
                setOneStateKnot(i + other->start_i - this->start_i, other->t_knots[i], other->ort_delta[i]);
            } else {
